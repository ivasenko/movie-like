...
const MovieItems = ({ state: { movies, displayCountry } }) => (
        <div>
            {movies.filter(({ country }) =>
                displayCountry === country || displayCountry === "all"
                )
                .map(({ country, title, img, year }) => (
                    <MovieItem key={`MovieItems-${title}`} country={country} title={title} img={img} year={year} />
                ))}
        </div>
...
<div>
    {movies.filter(({ year }) =>
        ...
        .map(({ country, title, img, year }) => (
            <MovieItem key={`MovieItems-${title}`} country={country} year={year} title={title} img={img} />
        ))}
</div>
);
...
const ButtonCategories = (movieCategories, setCountry ) => (
    movieCategories.map((country) => (
        <button key={country} className={`btn-${country}`} onClick={() => setCountry(country)}>
            {country}
        </button>
    ))
);
const ButtonCategoriesByYear = (movieCategoriesByYear, setMovieByYear ) => (
    movieCategoriesByYear.map((year) => (
        <button key={year} className={`btn-${year}`} onClick={() => setMovieByYear(year)}>
            {year}
        </button>
    ))
);
...
export class Filter extends Component {
    ...
    render() {
        return <UI setCountry={this.setCountry} state={this.state}  setMovieByYear={this.setMovieByYear}/>;
    }
}

// get unique country items
const uniqueItems = (x, i, array) => array.indexOf(x) === i;
const movie_category = movieData.map(film => film.country).filter(
    uniqueItems
);
const movie_category_by_year = movieData.map(film => film.year).filter(
    uniqueItems
);
